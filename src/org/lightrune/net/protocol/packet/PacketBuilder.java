...
public class PacketBuilder extends Packet {
	...
	public PacketBuilder putMESmallInt(int value) {
		putByte(value >> 16).putByte(value >> 24).putByte(value).putByte(value >> 8);
		return this;
	}
	...
	public PacketBuilder putBits(int amount, int value) {
		...
		for (; amount > bitOffset; bitOffset = 8) {
			...
			i = buffer().get(bytePos) | (value >> (amount - bitOffset)) & BIT_MASK[bitOffset];
			buffer().put(bytePos++, (byte) i);
			amount -= bitOffset;
		}
		if (amount == bitOffset) {
			...
			i = buffer().get(bytePos) & ~(BIT_MASK[amount] << (bitOffset - amount));
			buffer().put(bytePos, (byte) i);
			i = buffer().get(bytePos) | (value & BIT_MASK[amount]) << (bitOffset - amount);
			buffer().put(bytePos, (byte) i);
		}
		return this;
	}
	...
	public static PacketBuilder allocate(int capacity) {
		return (PacketBuilder) new PacketBuilder().buffer(ByteBuffer.allocateDirect(capacity));
	}
