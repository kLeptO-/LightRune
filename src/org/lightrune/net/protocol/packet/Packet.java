...
public class Packet {
	...
	public Packet sendTo(SocketChannel channel) {
		try {
			...
			World.deregister(((RSChannelContext)
					channel.keyFor(NIOSelector.selector()).attachment()).player());
		}
		return this;
	}
	...
	public static Packet buildPacket(ByteBuffer source, int opcode, int length) {
		Packet packet = new Packet().buffer(ByteBuffer.allocateDirect(length)).opcode(opcode).length(length);
		for (; length > 0; length--) {
